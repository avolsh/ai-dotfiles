...
### 2026-08-30 — two specs for one rule, and the narrower one never noticed the other had shipped

- **Spec / task:** IMP-20260826-plan-file-count-realism / pre-Plan
- **Category:** `process`
- **What was found:** `IMP-20260826-plan-file-count-realism` and `IMP-20260826-decomposition-and-staleness-procedures` were both written on 2026-08-26, from the same improvements-log entries, against the same target — what the ≤5 cap in `authoring-steps.md § C` counts. The wider one reached `done` on 2026-08-27 and landed the rule; the narrower one sat at `specify` for four days describing a `## Current State` that had stopped being true the moment its twin closed. It cited three overruns; its twin cited the same three plus a fourth. Nothing caught the overlap: neither spec lists the other in `siblings:` or `depends-on:`, so the `depends-on:` staleness rule the closed spec itself introduced (FR-6) never fired on the one spec in the corpus that most needed it. The overlap only surfaced when a human asked what the open spec was for and the Plan preconditions were checked against HEAD — where three of its four FRs were already satisfied, one of them by text the closed spec had made *stronger* (four exclusions, not two).
- **What was changed:** the narrower spec closed at `specify` with FR-1/FR-2/FR-4 tombstoned in place per `spec-lifecycle.md § Rules #10` and each AC re-checked against HEAD in `## Closure Evidence`; archived. No framework text changed — the rule it asked for is already there.
- **Suggested follow-up:** two separate gaps. (1) `validate-specs.py` already parses every active spec's front-matter; it does not compare their *targets*. Two active specs naming the same file in `affected-docs` is not an error, but two naming the same file and the same section is worth a warning at the requirements gate — this pair shared `authoring-steps.md` and `docs/writing-specs.md` outright. Cheaper still: warn when two active specs share a `date:` and any `affected-docs` entry. (2) The staleness rule is keyed to `depends-on:`, which is filled by the Split check — so it only protects specs that were split *from* each other. Two specs written independently against one rule are exactly the case it misses, and the case where `## Current State` rots fastest. The re-verification trigger may want a second key: any spec whose `affected-docs` were touched by a spec that reached `done` after this spec's `date:`.
- **Residual from the closed spec (FR-3):** a task row whose Files column runs well past five now has nothing recording *why* — the landed rule made the column and the cap measure different things, which removes the false split signal but also removes the reader's ability to tell decision count from convention spillover. Worth one clause in `authoring-steps.md § C` step 5 if a reader is ever confused by a long Files column; not worth a spec on its own.

### 2026-08-30 — one person, three `owner:` handles, and nothing checks the field

- **Spec / task:** IMP-20260830-active-spec-overlap-detection / Specify
- **Category:** `tooling`
...
### 2026-09-02 — A stale frame left standing in the current Figma file, for the second time

- **Spec / task:** IMP-20260902-run-cap-vs-monthly-ceiling (tobevisit-content), Visualize
- **Category:** `anti-pattern`
- **What was found:** Visualize cloned `[W-11] Ingestion · Step 1 — Console` to draw two proposed states and noticed, while cloning it, that the base frame was factually wrong about today's product: it read `Request budget · 1,000 / run · content.google.places.requests`, a configuration key deleted twenty days earlier and a unit renamed with it. The agent recorded the divergence in the spec's `## Design` and left the frame as it was, reasoning that archived specs link to it. That reasoning is the versioning scheme read backwards: archived specs resolve against **frozen** keys, which is precisely why the current file is free to be corrected — the owner had to point this out, and said it was the second occurrence. The current file is the live design; a stale frame in it is not "the before picture", because freezing is what preserves a before.
- **What was changed:** `framework/prompts/references/figma-file-organization.md` § 6 — new subsection "The current file carries no divergence" (correcting a stale base frame is part of the run that found it; correcting the base and drawing the proposal are two different acts, and an unapproved proposal is never written into the base frame; at closure the base frame gets what landed; the archived-specs argument named and refused), plus a § 7 checklist line. `framework/prompts/visualize-spec.prompt.md` — the same as a hard rule, since that is what an agent reads mid-run. `[W-11]` itself corrected to today's truth. This entry.
- **Suggested follow-up:** The rule is now written in the two places an agent reads, but nothing checks it — a stale frame is only visible to someone who knows the product. The cheap half is narrower than it looks: frames routinely quote configuration key paths as caption text, and a project's `settings-schema.ts` (or equivalent) declares every valid path, so a check could flag a key path rendered in the current file that the schema does not declare. That is the same shape as the check proposed the same day in `tobevisit-content` for `docs/domain/`, which argues for solving it once over both corpora rather than twice.

### 2026-09-02 — The placement assertions pass on a page whose sections do not line up

- **Spec / task:** IMP-20260902-run-cap-vs-monthly-ceiling (tobevisit-content), Visualize
- **Category:** `anti-pattern`
- **What was found:** The owner looked at the canvas and saw the `Catalog` section indented against every other section. The geometry audit came back clean — no section overlapping another, no child escaping its host — because that is exactly what § 5's placement recipe checks: each section against its own children, and each section box against its neighbours' boxes. Neither comparison involves a grid, so a page whose sections start at `x = 0, 0, 0, 0, 0, 0, 874` and sit at gutters of 130 to 360 passes every assertion the framework asks for and still looks wrong to the first human who scrolls it. The assertions are per-write and the defect is per-page; a check scoped to the frame you just added can only ever prove the frame you just added. A second, smaller thing surfaced in the same run: a strip appended to a screen's `Main` wrapper was positioned with an `x` read off a frame-relative dump, so it overflowed the screen by exactly the sidebar's width — two coordinate spaces in one frame, and the symptom looks like a sizing bug.
