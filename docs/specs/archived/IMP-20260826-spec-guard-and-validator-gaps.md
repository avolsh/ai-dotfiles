...
- FR-9: `validate-specs.py` MUST report an `affected-code:` or `affected-docs:` path that does not resolve from the spec's own project root, since `spec-status-guard.sh` matches paths in exactly that form and a path written any other way is a lease the guard cannot see. It MUST NOT report an entry that resolves from an ancestor directory into a *different* project — a cross-repo spec has no project-root-relative form for a sibling repo's file. *(Second sentence amended at closure; rationale in Closure Evidence.)*
...
| AC-6 | `validate-specs.test.sh`: `env/ai-dotfiles/framework/hooks/spec-status-guard.sh` in an `ai-dotfiles` spec is reported as unresolvable and exits 1; annotated / elliptical / trailing-slash paths under an existing tree stay clean. First run over `ai-dotfiles` produced **17 findings across 2 archived specs**; 15 were rewritten to project-root form, 2 were cross-repo and are exempt under FR-9's amended second sentence. | met |

**Benefit metrics (before → after).** Eight logged guard collisions replayed as fixtures: 8 denials → 1, the survivor being the occurrence the log itself records as a genuine sequencing conflict "the two proposed hook rules would not fix". `docs/domain/` files checked for REQ-ID collisions: 0 → all. Guard-invisible inventory paths in `ai-dotfiles`: 17 → 0. Registered validator checks: 11 → 13.

**Requirement amended at closure (FR-9).** As originally written, FR-9 reported any inventory path not resolving from the spec's own project root. Applied literally it flags the two cross-repo entries in `IMP-20260820-figma-screenshot-durability-and-file-versioning` (`affected-repos:` names both repos) that point into `tobevisit-content`. Those have no project-root-relative form: written as `docs/architecture/design-system.md` they would falsely match `ai-dotfiles`' own `docs/` tree, which is a worse lease than none. FR-9 therefore gained a second sentence exempting an entry that resolves from an ancestor into a *different* project, while still reporting one that resolves back into this project by a longer route — the silent-lease defect FR-9 names. AC-6's fixture is unaffected either way.
