...
Second defect, independent of expiry: node links point at a **live** file key. `src/github.com/tobeverse/tobevisit-content/docs/architecture/design-system.md:11` already declares the key exactly once and forbids retro-editing archived specs, but nothing freezes the file the archived specs were written against, so their links resolve to designs that have since changed.
