...
- FR-2: Every workflow prompt that delegates to those three (`create-spec.prompt.md`, `plan-spec.prompt.md`, `research-spec.prompt.md`) MUST drop the delegation/fallback and reference the absorbed skill sections instead.
...
| T3 | Delete the four bespoke agent files; rewrite `agents/README.md` to drop them and state the subagent-need gate (mechanical need only; reject role decomposition) (FR-1, FR-3). | `framework/agents/spec-author.md`, `framework/agents/splitter.md`, `framework/agents/task-planner.md`, `framework/agents/precedent-finder.md`, `framework/agents/README.md` | — | T2 | writing-specs | default | ✅ done |
| T4 | Bring the rest of the framework into line with the removal: rewrite `research-spec.prompt.md` to inline RES authoring (`§ D`), reroute the task-start preflight to Grep/Glob or the built-in explore subagent, update the delegation docs to inline authoring, and update both validators so `validate-specs` no longer assumes the removed agents and `lint-rules` no longer flags the absorbed inline Steps as drift; `make sync-agents-check` passes (FR-2, FR-3, FR-4). | `framework/prompts/research-spec.prompt.md`, `framework/spec-workflows/spec-types.md`, `docs/agent-protocol.md`, `scripts/validate-specs.py`, `scripts/lint-rules.py` | `framework/agents/README.md`, `framework/boundaries.md` | T3 | writing-specs | default | ✅ done |
