...
Discovers specs under `docs/specs/{active,archived}/` and agents under
`framework/agents/`, then runs the rules declared in
`framework/spec-workflows/lifecycle.yaml` through `lifecycle_engine.py`
(IMP-20260917-lifecycle-schema-engine). The checks that read document structure
stay here as plug-ins: `PLUGINS` below, each named in the schema's `plugins:`
with the finding ids it owns. Findings print as `path:lineno:check:message`;
the exit code is 1 when any finding is emitted and 2 when the schema does not
resolve.

Stdlib only (IMP-20260514-spec-validator FR-7): front matter is read by
`speclib._parse_front_matter`, the schema by `yamlite.py`. The repository root
is found by walking up from the given path (default: this script) until a
...
# ---------------------------------------------------------------------------
# Plug-ins — checks kept as code; lifecycle.yaml declares every other rule
# ---------------------------------------------------------------------------
...
def _has_tasks_table(spec: Spec) -> bool:
    """True iff `## Tasks` holds an actual table outside any fenced block.

    Prose-only Tasks sections ('Pending — Plan stage only.') and a table shown
    as a fenced example are not tables (IMP-20260917-lifecycle-schema-engine D4 #3).
    """
    fence: str | None = None
...
def check_status_invariants(specs: Iterable[Spec]) -> list[Finding]:
    ...
    for spec in specs:
        ...
        if status == "specify" and _has_tasks_table(spec):
            findings.append(
                Finding(
                    spec.path,
                    line,
...
def _closure_date(spec: Spec) -> tuple[_dt.date, str] | None:
    ...
    for pattern, source in (
        ...
        (_LAST_UPDATED_RE, "Last updated stamp"),
    ):
...
def main(argv: list[str]) -> int:
    ...
    try:
        schema = lifecycle_engine.load_schema(PLUGINS)
    except lifecycle_engine.SchemaError as exc:
        print(f"validate-specs: lifecycle.yaml does not resolve: {exc}", file=sys.stderr)
        return 2
    findings.extend(lifecycle_engine.run(root, {"specs": specs, "agents": agents}, PLUGINS, schema=schema))
    ...
    rules = len({r["id"] for r in schema["rules"]})
    if findings:
        print(
    ...
    print(
        ...
        f"{rules} rule(s) + {len(PLUGINS)} plug-in(s)).",
        file=sys.stderr,
    )
    return 0
