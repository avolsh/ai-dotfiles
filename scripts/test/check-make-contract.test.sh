...
makefile 'core code' </dev/null > "$d/sub/Makefile"
expect_one "cd into a directory, then make an undefined target" "$d" "rule-4 deps-fix"
printf '%s' "$out" | grep -q "undefined target 'nope' in sub" || { echo "FAIL: cross call names sub ($out)" >&2; fails=$((fails + 1)); }
d="$(printf 'deps-fix:\n\t$(MAKE) -C sub nope\n' | repo r4m core)"; mkdir -p "$d/sub"
makefile 'core code' </dev/null > "$d/sub/Makefile"
expect_one "-C into a component, undefined target" "$d" "rule-4 deps-fix"
expect_one "-C into a directory without a Makefile" "$(printf 'deps-fix:\n\t$(MAKE) -C nowhere build\n' | repo r4n core)" \
  "rule-4 deps-fix"
d="$(printf 'deps-fix:\n\t$(MAKE) -C _lib/shared build\n' | repo r4o core)"; git -C "$d" init -q; printf '_lib/\n' > "$d/.gitignore"
expect_ok "-C into a library not yet installed" "$d"
expect_ok "quoted separators stay in the echo" "$(printf 'deps-fix:\n\t@echo "usage: make build; make nope && make gone"\n' | repo r4f core)"
expect_one "make inside if/then" "$(printf 'deps-fix:\n\t@if true; then make nope; fi\n' | repo r4g core)" "rule-4 deps-fix"
expect_ok "make output redirected" "$(printf 'deps-fix:\n\t$(MAKE) clean > out.log\n' | repo r4h core)"
d="$(printf 'deps-fix:\n\t(cd sub && $(MAKE) lint)\n' | repo r4i core)"; mkdir -p "$d/sub"
makefile 'core code' </dev/null > "$d/sub/Makefile"
expect_ok "subshell cd, then make" "$d"
d="$(printf 'deps-fix:\n\tcd sub && \\\n\t  $(MAKE) lint\n' | repo r4j core)"; mkdir -p "$d/sub"
makefile 'core code' </dev/null > "$d/sub/Makefile"
expect_ok "cd, continuation line, then make" "$d"
d="$(printf 'wipe-tmp:\n\tcd lib && bash _dev/run.sh\n' | repo r5j core)"; mkdir -p "$d/lib/_dev"; : > "$d/lib/_dev/run.sh"
expect_ok "cd, then run a script that exists there" "$d"
expect_one "cd, then run a script missing there" "$(printf 'wipe-tmp:\n\tmkdir -p lib; cd lib && bash _dev/run.sh\n' | repo r5k core)" \
  "rule-5 wipe-tmp"
expect_one "script inside if/then" "$(printf 'wipe-tmp:\n\t@if true; then bash _dev/missing.sh; fi\n' | repo r5i core)" "rule-5 wipe-tmp"
expect_ok "numbered redirects" "$(printf 'deps-fix:\n\t$(MAKE) clean 2>/dev/null; $(MAKE) clean >/dev/null 2>&1; $(MAKE) clean 2> err.log\n' | repo r4k core)"
d="$(printf 'deps-fix:\n\t(cd sub && $(MAKE) lint); $(MAKE) nope\n' | repo r4l core)"; mkdir -p "$d/sub"
makefile 'core code' </dev/null > "$d/sub/Makefile"
expect_one "cd does not outlive its subshell" "$d" "rule-4 deps-fix"
