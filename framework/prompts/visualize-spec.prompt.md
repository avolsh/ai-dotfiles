...
## Hard rules
- Status stays at `specify` (Visualize is a sub-step, not a status — see [`spec-lifecycle.md § Rules #8`](../spec-workflows/spec-lifecycle.md#visualize-not-a-status)). Diagrams MUST match approved requirements, not aspiration. One-line captions only — never paraphrase the diagram in prose. No `## Tasks` table here — see [`spec-lifecycle.md § Rules #2`](../spec-workflows/spec-lifecycle.md#never-tasks-table-at-specify).
