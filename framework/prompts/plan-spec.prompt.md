...
2a. **Ask `spec-next`** — after the flip, run `python3 "$AI_DOTFILES/scripts/spec-next.py" <spec>`; its output names the decomposition procedure, the task-table format, the safety net and the plan gate for this spec, with the files to load under `Load:`. Use that instead of reading the docs linked in steps 3–5 in full. **Fallback:** if the command is missing or exits non-zero, say so and read those docs in full.
